Public Class Form1
    ...
    Sub Room1(start As Integer)
        ...
        TileLayer(2, 10, ImageList1.Images(16))

        TileLayer(5, 8, ImageList1.Images(16))
        TileLayer(2, 4, ImageList1.Images(16))
        TileLayer(10, 10, ImageList1.Images(16))
        TileLayer(10, 8, ImageList1.Images(16))
        TileLayer(10, 4, ImageList1.Images(16))
        TileLayer(5, 10, ImageList1.Images(16))
        TileLayer(5, 4, ImageList1.Images(16))
        TileLayer(7, 1, ImageList1.Images(15))
    ...
    Sub SetTile1(x As Integer, y As Integer, index As Integer)
        ...
        m_Game(levelindex, roomindex, x, y).SetBackGround(index)
    End Sub
    Sub TileLayer(x As Integer, y As Integer, maper As Bitmap)
        maper.MakeTransparent(Color.White)
        Dim bmp As New Bitmap(Math.Max(24, 24), 24)
        Dim g As Graphics = Graphics.FromImage(bmp)
        Dim tile As Image = ImageList1.Images(m_Game(levelindex, roomindex, x, y).ReturnBackGround)
        g.DrawImage(tile, 0, 0, 24, 24)
        g.DrawImage(maper, 0, 0, 24, 24)
        g.Dispose()
        m_buttonArray(x, y).BackgroundImage = bmp
        m_Game(levelindex, roomindex, x, y).SetIndex(WallTileIndex)
    End Sub
